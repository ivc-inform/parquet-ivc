 ...
 * {@link ru.mfms.parquet.avro.AvroIndexedRecordConverter} and works with generic, specific, and
 * reflect records.
 *
...
class AvroRecordConverter<T> extends AvroConverters.AvroGroupConverter {
  ...
  public AvroRecordConverter(MessageType parquetSchema, Schema avroSchema,
    ...
    this.rootContainer = ParentValueContainer.getConversionContainer(new ParentValueContainer() {
      ...
      public void add(Object value) {
        ru.mfms.parquet.avro.AvroRecordConverter.this.currentRecord = (T) value;
      }
  ...
  public AvroRecordConverter(ParentValueContainer parent,
    ...
    for (Type parquetField: parquetSchema.getFields()) {
      ...
      ParentValueContainer container = new ParentValueContainer() {
        @Override
        public void add(Object value) {
          ru.mfms.parquet.avro.AvroRecordConverter.this.set(avroField.name(), finalAvroIndex, value);
        }
      };
      ...
      if (recordClass != null &&
          converters[parquetFieldIndex] instanceof AvroConverters.FieldStringConverter) {
        try {
          Field field = recordClass.getDeclaredField(avroField.name());
          if (field.isAnnotationPresent(Stringable.class)) {
            converters[parquetFieldIndex] = new AvroConverters.FieldStringableConverter(
  ...
  private static Converter newConverter(Schema schema, Type type,
    ...
    if (schema.getType().equals(Schema.Type.BOOLEAN)) {
      ...
      return new ru.mfms.parquet.avro.AvroRecordConverter(parent, type.asGroupType(), schema, model);
