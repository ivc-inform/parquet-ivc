...
public class AvroReadSupport<T> extends ReadSupport<T> {
  ...
  /**
   * @see ru.mfms.parquet.avro.AvroParquetInputFormat#setRequestedProjection(org.apache.hadoop.mapreduce.Job, org.apache.avro.Schema)
   */
  ...
  /**
   * @see ru.mfms.parquet.avro.AvroParquetInputFormat#setAvroReadSchema(org.apache.hadoop.mapreduce.Job, org.apache.avro.Schema)
   */
  ...
  public RecordMaterializer<T> prepareForRead(
    ...
    String compatEnabled = metadata.get(ru.mfms.parquet.avro.AvroReadSupport.AVRO_COMPATIBILITY);
    if (compatEnabled != null && Boolean.valueOf(compatEnabled)) {
      return newCompatMaterializer(parquetSchema, avroSchema, model);
    }
    return new ru.mfms.parquet.avro.AvroRecordMaterializer<T>(parquetSchema, avroSchema, model);
  }
  ...
  private static <T> RecordMaterializer<T> newCompatMaterializer(
    ...
    return (RecordMaterializer<T>) new ru.mfms.parquet.avro.AvroCompatRecordMaterializer(
